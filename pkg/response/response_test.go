...
func (s *responseSuite) TestResponse_WithMessage() {
	type fields struct {
		Code int
		Msg  string
		Data interface{}
...
func (s *responseSuite) TestResponse_WithData() {
	type fields struct {
		Code int
		Msg  string
		Data interface{}
...
func (s *responseSuite) TestResponse_String() {
	type fields struct {
		Code int
		Msg  string
		Data interface{}
	...
	tests := []struct {
		...
		{
			name:   "to json string then success",
			fields: fields{Code: 200, Msg: "message", Data: nil},
			want:   `{"code":200,"msg":"message"}`,
		},
	}
	for _, tt := range tests {
		s.T().Run(tt.name, func(t *testing.T) {
